...
const SELECTABLE_COLS = [
  ...
  { key: 'create_time', title: '创建时间', width: 160, getValue: (r: any) => r.create_time || '-' },
];
...
const Component = () => {
  ...
  // 已应用的筛选条件：仅在点击「查询」后生效，输入过程不会实时筛选
  ...
  const filteredData = useMemo(() => {
    return dataSource.filter((item) => {
      ...
      if (appliedFilter.iossNo.length > 0 && !appliedFilter.iossNo.some((v) => item.ioss_no.includes(v))) return false;
      if (appliedFilter.auditType && item.audit_type !== appliedFilter.auditType) return false;
      if (appliedFilter.auditStatus && item.audit_status !== appliedFilter.auditStatus) return false;
  ...
  const handleReset = () => {
    ...
    setAppliedFilter({ customerCode: '', iossNo: [], auditType: undefined, auditStatus: undefined, createTime: null });
  ...
  const renderCell = (key: string) => {
    switch (key) {
      ...
      case 'audit_status': return (v: any) => <CopyableCell text={renderStatus(v)} />;
  ...
  return (
    <div className="ioss-layout">
      ...
      <div className="ioss-action-bar">
        <Space>
          <Button onClick={handleApprove} disabled={selectedRowKeys.length === 0}>审核通过</Button>
          <Button onClick={handleReject} disabled={selectedRowKeys.length === 0}>审核不通过</Button>
