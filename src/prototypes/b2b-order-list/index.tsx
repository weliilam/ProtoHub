...
import {
  ...
  InputNumber, Divider, Descriptions, Card,
} from 'antd';
import {
  SearchOutlined, ReloadOutlined, DownOutlined,
  SettingOutlined, DeleteOutlined, CopyOutlined,
...
const B2BOrderList = () => {
    ...
    { title: '入账状态', dataIndex: 'billStatus', key: 'billStatus', width: 100, align: 'center' },
    {
      ...
      render: (_: any, record: OrderRecord) => (
        <Space size={0} split={<span className="bol-action-divider">|</span>}>
          <Button type="link" size="small" onClick={() => { setDetailRecord(record); setDetailOpen(true); }}>详情</Button>
          <Button type="link" size="small" onClick={() => { setAuditRecord(record); setAuditReason(''); setAuditOpen(true); }}>审核</Button>
          <Button type="link" size="small" onClick={() => { setEditRecord(record); setEditOpen(true); }}>编辑</Button>
          <Button type="link" size="small" onClick={() => { setLogRecord(record); setLogOpen(true); }}>日志</Button>
  ...
  }, [handleExportFollowUp, handleExportDetention, handleExportType]);
