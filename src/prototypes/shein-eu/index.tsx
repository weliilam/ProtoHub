import { ConfigProvider, Tabs, Card, Table, Tag, Typography, Descriptions, Alert, Space, Row, Col, Progress } from 'antd';
...
const SEU_STYLE = `
  ...
  .seu-overview-table .ant-table-content::-webkit-scrollbar {
    height: 8px;
  }
  .seu-overview-table .ant-table-content::-webkit-scrollbar-thumb {
    background: #cfd6df;
    border-radius: 4px;
  }
  .seu-overview-table .ant-table-content::-webkit-scrollbar-track {
    background: transparent;
  }
`;
...
interface SectionSummary {
  ...
  no: number;
  required: number;
  rate: number; // 可映射占比 = (ok + partial) / total
}

function summarizeSections(): SectionSummary[] {
  return MAPPING_SECTIONS.map((s) => {
    const total = s.rows.length;
    const ok = s.rows.filter((r) => r.status === 'ok').length;
    const partial = s.rows.filter((r) => r.status === 'partial').length;
    const no = s.rows.filter((r) => r.status === 'no').length;
    const required = s.rows.filter((r) => r.required === true).length;
    return {
      title: s.title,
      total,
      ok,
      ...
      no,
      required,
      rate: total ? Math.round(((ok + partial) / total) * 100) : 0,
    };
  });
}

const SECTION_SUMMARY = summarizeSections();

const OVERVIEW_STATS = SECTION_SUMMARY.reduce(
  (acc, s) => ({
  ...
  { total: 0, ok: 0, partial: 0, no: 0, required: 0 },
);

// 待确认事项：必填但未完全映射 / 说明中含「待SHEIN提供 / 需货代确认 / 待确认」等关键字
const PENDING_KEYWORDS = ['待SHEIN', '需货代', '待确认', '需要确定', '是否需要', '留空'];
...
function collectPendingItems(): PendingItem[] {
  ...
  MAPPING_SECTIONS.forEach((s) => {
    s.rows.forEach((r) => {
      const hasPendingKeyword = PENDING_KEYWORDS.some((kw) => r.note.includes(kw));
      const isBlocking = r.required === true && r.status !== 'ok';
      if (hasPendingKeyword || isBlocking) {
        items.push({
          key: `${s.title}-${r.key}`,
          section: s.title,
          field: r.ytField,
...
const PENDING_ITEMS = collectPendingItems();

const rateColor = (rate: number) => (rate >= 80 ? '#16a34a' : rate >= 50 ? '#d97706' : '#dc2626');
      ...
      <div className="seu-page" style={{ padding: 24, minHeight: '100vh' }}>
        ...
        <Tabs
          ...
          items={[
            {
              key: 'overview',
              label: <span><PieChartOutlined /> 对接概览</span>,
              children: (
                <Card variant="borderless" className="seu-card">
                  <Alert
                    ...
                    message={<Text strong>字段映射健康度总览</Text>}
                    description="自动汇总「下单字段映射」数据：各分组映射覆盖情况、必填字段缺口与待确认事项，便于快速定位对接阻塞点。"
                  />

                  {/* 总体统计卡片 */}
                  <Row gutter={[12, 12]} style={{ marginBottom: 24 }}>
                    {[
                      { label: '字段总数', value: OVERVIEW_STATS.total, color: '#1677ff' },
                      { label: '可映射', value: OVERVIEW_STATS.ok, color: '#16a34a', suffix: '✅' },
                      { label: '部分映射', value: OVERVIEW_STATS.partial, color: '#d97706', suffix: '⚠️' },
                      { label: '无对应字段', value: OVERVIEW_STATS.no, color: '#dc2626', suffix: '❌' },
                      { label: '必填字段', value: OVERVIEW_STATS.required, color: '#4f46e5' },
                      { label: '待确认事项', value: PENDING_ITEMS.length, color: '#ea580c', suffix: '项' },
                    ].map((c) => (
                      <Col xs={12} md={8} lg={4} key={c.label}>
                        <div className="seu-stat-card">
                          <Text type="secondary" className="seu-stat-label">{c.label}</Text>
                          <span className="seu-stat-num" style={{ color: c.color }}>
                            {c.value}
                  ...
                  {/* 各分组覆盖情况 */}
                  <div className="seu-section-head">
                    <Text className="seu-section-title">各分组字段映射覆盖</Text>
                    <Text type="secondary" className="seu-section-desc">可映射率 =（可映射 + 部分映射）/ 字段数</Text>
                  </div>
                  <Table<SectionSummary>
                    className="seu-overview-table"
                    size="small"
                    bordered
                    rowKey="title"
                    pagination={false}
                    columns={[
                      { title: '分组', dataIndex: 'title', width: 260 },
                      { title: '字段数', dataIndex: 'total', width: 80, align: 'center' },
                      { title: '✅ 可映射', dataIndex: 'ok', width: 90, align: 'center', render: (v: number) => <Text style={{ color: '#16a34a' }}>{v}</Text> },
                      { title: '⚠️ 部分', dataIndex: 'partial', width: 80, align: 'center', render: (v: number) => <Text style={{ color: '#d97706' }}>{v}</Text> },
                      { title: '❌ 无对应', dataIndex: 'no', width: 90, align: 'center', render: (v: number) => <Text style={{ color: '#dc2626' }}>{v}</Text> },
                      { title: '必填', dataIndex: 'required', width: 70, align: 'center' },
                      {
                        title: '可映射率',
                        dataIndex: 'rate',
                        width: 180,
                    ...
                    dataSource={SECTION_SUMMARY}
                    footer={() => (
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 24 }}>
                        <Text type="secondary">合计</Text>
                        <Text>字段 <Text strong>{OVERVIEW_STATS.total}</Text></Text>
                        <Text>✅ <Text strong style={{ color: '#16a34a' }}>{OVERVIEW_STATS.ok}</Text></Text>
                        <Text>⚠️ <Text strong style={{ color: '#d97706' }}>{OVERVIEW_STATS.partial}</Text></Text>
                        <Text>❌ <Text strong style={{ color: '#dc2626' }}>{OVERVIEW_STATS.no}</Text></Text>
                        <Text>待确认 <Text strong style={{ color: '#ea580c' }}>{PENDING_ITEMS.length}</Text></Text>
                      </div>
                    )}
                  />

                  {/* 待确认 / 阻塞事项 */}
                  <div className="seu-section-head" style={{ marginTop: 24 }}>
                    <Text className="seu-section-title">待确认 / 阻塞事项</Text>
                    <Text type="secondary" className="seu-section-desc">必填但未完全映射，或需外部确认映射关系的字段</Text>
                  </div>
                  <Table<PendingItem>
                    className="seu-overview-table"
                    size="small"
                    ...
                    pagination={false}
                    columns={[
                      { title: '所属分组', dataIndex: 'section', width: 220 },
                      { title: '云途字段', dataIndex: 'field', width: 240, render: (v: string) => <Text code>{v}</Text> },
                      {
                        title: '状态',
                        dataIndex: 'status',
                        width: 110,
                      ...
                      { title: '说明 / 待确认内容', dataIndex: 'note', render: (v: string) => <Text type="secondary">{v || '—'}</Text> },
                    ]}
