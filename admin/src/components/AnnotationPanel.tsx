...
export function buildAiPrompt(target: string, list: Annotation[]): string {
  const lines = list.map((a, i) => `${i + 1}. 元素 \`${a.selector}\`：${a.text}`);
...
export default function AnnotationPanel({ target, annotations, onToggleStatus, onDelete, onApplied, onMarkDone, orphanAnnotations = [], onDeleteOrphans, canUndo, onUndo }: Props) {
  ...
  // 批注列表变化时，把当前所有 open 批注默认勾选上（已取消勾选的会在下次变更时复位）
  useEffect(() => {
    setSelected(new Set(annotations.filter((a) => a.status === 'open').map((a) => a.id)));
  }, [annotations]);
      ...
      Modal.info({
        ...
        content: (
          <div>
            ...
            >
              <div style={{ fontWeight: 600, marginBottom: 6, color: '#0958d9' }}>
                📋 本次原型改动说明（给产品 / 业务同学）
              </div>
            ...
            <Collapse
              ...
              items={[
                {
                  key: 'detail',
                  label: <span style={{ color: '#888', fontSize: 13 }}>技术改动明细（开发参考，点击展开）</span>,
                  children: (
                    <div>
                      <div style={{ fontWeight: 600, marginBottom: 6, color: '#888' }}>代码改动（git diff）</div>
                      <pre style={DIFF_STYLE}>{diff || '（无代码变更，或项目尚未纳入 Git 版本管理）'}</pre>
                      <div style={{ fontWeight: 600, margin: '12px 0 6px', color: '#888' }}>AI 执行日志</div>
                      <pre style={{ ...DIFF_STYLE, background: '#f6f8fa', color: '#1f2328' }}>
    ...
    <>
      ...
      <div className="ph-right-panel-body">
        ...
        {orphanAnnotations.length > 0 && (
          ...
          >
            <div style={{ fontSize: 12, color: '#ad6800', marginBottom: 6 }}>
            ...
            {onDeleteOrphans && (
              <Button
                ...
                danger
                style={{ marginTop: 8 }}
                onClick={() => onDeleteOrphans(orphanAnnotations.map((a) => a.id))}
              >
                一键清理失效批注
              </Button>
            )}
        ...
        {publishing && (
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <LoadingOutlined style={{ fontSize: 18, color: '#1677ff' }} spin />
              <span style={{ fontSize: 13, fontWeight: 600, color: '#1677ff' }}>
                {liveOutput ? 'AI 正在改写原型文件…' : 'AI 正在分析批注意见…'}
              </span>
            </div>
            {/* 实时输出 */}
            <pre
