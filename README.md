...
- **命令面板（⌘K）**：`Ctrl/Cmd + K` 唤起，可跳转任意条目、打开 AI 助手 / Git 快照 / 批注列表、刷新预览、切换主题等。
- **一键复制与分享**：工具栏「复制」一键复制当前原型链接，自动将 localhost 替换为局域网 IP，发给同事 / 手机即可访问。
- **深色 / 浅色主题**：工具栏一键切换，命令面板亦可切换。
...
- 工具栏显示本机 IP 标签（局域网访问地址），AI 任务执行时显示「AI 执行中」呼吸灯；
- 工具栏右侧月亮 / 太阳按钮一键切换深色 / 浅色主题。

### 命令面板（Ctrl/Cmd + K）

- 任意界面按 `Ctrl/Cmd + K`（或点击侧边栏底部「全局搜索 / 命令」）唤起；
- 支持**跳转**：输入名称快速切换任意原型 / 文档 / 主题 / 数据表条目；
- 支持**操作**：打开 AI 助手、打开 Git 快照、打开批注列表、刷新预览、进入 / 退出批注模式、切换深色 / 浅色主题；
- 支持键盘上下键选择、回车执行，中文 / 英文关键词均可匹配。
...
- 「复制对比摘要」一键生成结构化文本（两版本 + 变更清单），可直接粘贴发给同事 / 记录到工单；
- 用于快速定位某次迭代的视觉差异，无需手动回滚比对。

### 复制与分享

- **复制页面链接**：工具栏「复制」按钮复制当前原型完整 URL，若本机用 localhost 访问会自动替换为局域网 IP（如 `http://192.168.x.x:5173/p/xxx`），同事 / 手机同网段即可直接打开；
- **复制对比摘要**：版本对比弹窗内「复制对比摘要」，包含两版本信息与变更清单；
- **复制 AI 指令**：批注面板把待处理批注整理为 AI 修改指令后复制（见上方「批注」）；
- **复制错误信息**：原型运行报错时错误浮层提供「复制错误信息」，一键复制堆栈粘贴给开发排查。

### AI CLI

- 顶部工具栏「AI」按钮或命令面板（`Ctrl/Cmd + K` → 打开 AI 助手）唤起右侧「AI 助手」面板，可调用已配置的 AI 命令行工具；
...
- 点击「刷新」重新挂载原型以查看最新改动；
- 点击「复制」生成含局域网 IP 的链接，发给同事在手机 / 其他电脑上同网段访问预览。
